...
The scripting area in Kephas handles the text generation out of various templates.
The entry point is the ```IScriptProcessor``` singleton service, which returns an execution result through the ```ExecuteAsync``` method, provided with a script and execution arguments.~~~~
